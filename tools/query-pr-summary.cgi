...
# NOTE: locally modified by linimon
...
if ($html_mode) {
	...
	if ($opt_s) {
        ...
        # XXX MCL 20080907 not informative
	#$input{'very_quiet'}	= 'yes' if $opt_Q;
	$input{'quiet'}		= 'yes' if $opt_q || $opt_Q;
	if ($opt_C) {
		$query_args   = '--confidential=yes ';
	} elsif (!$opt_c) {
		$query_args   = '--restricted ';
	}
	# tag, ignoring severity
	if ($opt_T) {
...
# XXX MCL moved to bottom 20080907
#       if ($html_mode) {
#	  print <<EOM;
#
#<p>
...
# XXX MCL 20080907 s/''/'summary'/
} elsif ($input{'severity'} eq 'summary' && $opt_T eq '') {
	&severity_summary;
...
# XXX MCL code moved here 20080907
       if ($html_mode) {
	  print <<EOM;
